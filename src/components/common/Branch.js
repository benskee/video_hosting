                     ...
                     <span className="folder" onClick={this.handleArrowClick}>{branchName}</span>
                        ...
                        if (!['folder','start'].includes(name)) {
                            if (treeData[name]["folder"] === true) {
                                ...
                                return <Leaf key={name} name={name} objectPath={`${objectPath}.${name}`} onSelect={onSelect} selectedFile={selectedFile} start={treeData[name]['start']} currentTime={currentTime}/>
                                }
                            } else {
                            return null
                        }
